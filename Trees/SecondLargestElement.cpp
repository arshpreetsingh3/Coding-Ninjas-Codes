...
TreeNode<int>* secondLargest(TreeNode<int> * root){
    ...
    pendingnodes.push(root);
    while(!pendingnodes.empty()){
        TreeNode<int> * currentroot = pendingnodes.front();
        if(currentroot->data > temp2){
            temp1 = temp2;
            SecondLargestNode = LargestNode;
            temp2 = currentroot->data;
            LargestNode = currentroot;
        }else if(currentroot->data >temp1 && currentroot->data <temp2){
            temp1=currentroot->data;
            SecondLargestNode=currentroot;
        }
        for(int i=0;i<currentroot->children.size();i++){
            pendingnodes.push(currentroot->children[i]);
        }
        pendingnodes.pop();
    }
